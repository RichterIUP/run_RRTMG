...
def albedo_ebert_curry(sic):
    func = lambda x: 1.99 * x + 0.01
    h = func(sic)
    if h < 1.0:
        return [0.760 + 0.140*np.log(h), 0.247+0.029*np.log(h), 0.055, 0.036]
    elif h >= 1.0 and h < 2.0:
        return [0.770 + 0.018*(h-1), 0.247 + 0.196*(h-1), 0.055, 0.036]
    else:
        return [0.778, 0.443, 0.055, 0.036]
...
def create_input_rrtmg_lw(height_prof, press_prof, t_prof, humd_prof, solar_zenith_angle, lat=75., clouds=0, semiss=0.99, co2=None, n2o=None, ch4=None):
    ...
    #              is used for band IB.
    SEMISS = [semiss]#np.array([0.99 for i in range(16)])#[semiss]
...
def create_input_rrtmg_sw(height_prof, press_prof, t_prof, humd_prof, solar_zenith_angle, lat=75., clouds=0, albedo_dir=0.3, albedo_diff=0.3, co2=None, n2o=None, ch4=None, juldat=150, siconca=1.0):
    # Record 1.1
    ...
    #              and no surface emissivity value is given for SEMISS(IB), a value of 1.0 
    #              is used for band IB.
    albedo_ice = albedo_ebert_curry(siconca)
    #print(albedo_dir, albedo_diff)
    #SEMISS_DIF = [1-albedo_dir]#[0.8]
    #SEMISS_DIR = [1-albedo_diff]
    SEMISS_DIR = np.array([0.0 for i in range(14)])
    SEMISS_DIR[np.array([0,1,13])] = albedo_ice[3]
    SEMISS_DIR[np.array([2,3,4,5,6])] = albedo_ice[2]
    SEMISS_DIR[np.array([7,8])] = albedo_ice[1]
    SEMISS_DIR[np.array([9,10,11,12])] = albedo_ice[0]
    SEMISS_DIF = SEMISS_DIR
...
def RRTMG(z, p, t, q, sza, albedo_dir, albedo_diff, cloud, cwp, rl, ri, wpi, semiss, lat, co2, n2o, ch4, clt, clouds, juldat, siconca):
    '''
    Call RRTMG
    '''
    ret = create_input_rrtmg_sw(height_prof=z, \
                           ...
                           albedo_dir=albedo_dir, \
                               ...
                               lat=lat, co2=co2, n2o=n2o, ch4=ch4, juldat=juldat, siconca=siconca)
    if clouds != 0:
...
def write_results(all_sw, all_lw, clear_sw, clear_lw,  deriv_cwp_lw, deriv_cwp_sw, deriv_rl_lw, deriv_rl_sw, deriv_ri_lw, deriv_ri_sw, deriv_wpi_lw, deriv_wpi_sw, lat, lon, sza, cwp, wpi, rl, ri, cloud, z, t, q, p, dcwp, dwpi, drl, dri, co2, n2o, ch4, albedo_dir, albedo_diff, iceconc, semiss, clt, diff_lwp, flag_lwc, flag_iwc, flag_reff, flag_reffice, fname):
    '''
    ...
    with nc.Dataset(fname, "w") as outfile:
        ...
        for ii in range(len(all_sw.keys())):
            ...
            deriv_cwp_sw_out[ii][:] = np.array(deriv_cwp_sw[all_sw.keys()[ii]][::-1])
            deriv_rl_sw_out[ii][:] = np.array(deriv_rl_sw[all_sw.keys()[ii]][::-1])
            deriv_ri_sw_out[ii][:] = np.array(deriv_ri_sw[all_sw.keys()[ii]][::-1])
            deriv_wpi_sw_out[ii][:] = np.array(deriv_wpi_sw[all_sw.keys()[ii]][::-1])
        ...
        for ii in range(len(all_lw.keys())):
            ...
            deriv_cwp_lw_out[ii][:] = np.array(deriv_cwp_lw[all_lw.keys()[ii]][::-1])
            deriv_rl_lw_out[ii][:] = np.array(deriv_rl_lw[all_lw.keys()[ii]][::-1])
            deriv_ri_lw_out[ii][:] = np.array(deriv_ri_lw[all_lw.keys()[ii]][::-1])
            deriv_wpi_lw_out[ii][:] = np.array(deriv_wpi_lw[all_lw.keys()[ii]][::-1])
...
def main(fname_in, fname_out="out.nc", ERA5_PRESSURE=None):
    ...
    clear_sw, clear_lw = RRTMG(z, p, t, q, sza, albedo_dir, albedo_diff, cloud, cwp, rl, ri, wpi, semiss, lat, co2, n2o, ch4, clt, 0, juldat=(dt.datetime.strptime(fname.split("_")[2], "%Y%m%d") - dt.datetime(2017, 1, 1)).days, siconca=iceconc)
    ...
    all_sw, all_lw = RRTMG(z, p, t, q, sza, albedo_dir, albedo_diff, cloud, cwp, rl, ri, wpi, semiss, lat, co2, n2o, ch4, clt, 2, juldat=(dt.datetime.strptime(fname.split("_")[2], "%Y%m%d") - dt.datetime(2017, 1, 1)).days, siconca=iceconc)
